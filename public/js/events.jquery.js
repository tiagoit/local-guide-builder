...
$('.partial-description-span').each((i, el) => {
	console.log($(el).html($(el).text().substring(0, 200)));
	...
	$('.ad-card > .card-actions > a').click(ev => {
		let adId = $(ev.target).data('ad-id');
		console.log(adId, 'isMobile', isMobile);
		$.post('/api/app/track-ad-click', {adId, isMobile: isMobile()}, result => {
